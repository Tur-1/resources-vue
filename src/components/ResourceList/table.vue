...
<template>
  ...
  <div class="d-flex flex-wrap align-items-start justify-content-between mt-4">
    ...
    <div class="col-lg-8 col-md-7 d-flex justify-content-end flex-wrap gap-2 flex-grow-1">
      <RouterLink  v-for="action in props.resource.headerActions()"
      :to="{ name: action.routeName}" :class="action.class"
    ...
    <div class="card-body p-3">
      <div class="table-responsive">
        <table class="table table-hover">
          ...
          <tbody>
            <TableRow v-if="!useTableSkeletonLoading.isLoading" @openConfirm="openConfirmModal"
              :data="resourceDataList.list.value" :pages="props.resource.pages()" :columns="props.resource.fields()"
              :actions="props.resource.actions()" />
            <TableSkeleton v-if="useTableSkeletonLoading.isLoading" :columnsLength="props.resource.fields().length" />
            <NoRecordsFound v-if="
      ...
      <TablePagination v-if="resourceDataList.pagination.value?.length != 0"
        :simplePagination="props.resource.simplePagination" :paginationQueryKey="props.resource.paginationQueryKey" />
    </div>
  </div>
